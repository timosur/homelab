...
The cluster runs two separate Envoy Gateway instances — one for **home/LAN** traffic and one for **internet-facing** traffic. DNS is managed externally by the Unifi router, which updates the `*.timosur.com` wildcard record in Cloudflare to point to the correct public IP. TLS certificates for internet services are provisioned automatically via cert-manager with Let's Encrypt DNS-01 challenges through Cloudflare.

## Architecture

```
Internet → Unifi Router (port forward) → 192.168.2.254 → envoy-gateway-internet → Internet Services
LAN Client → 192.168.2.100 → envoy-gateway-home → Home Services
```

### Traffic Flow

1. **Internet traffic**: The Unifi router forwards ports 80/443 to `192.168.2.254`, the Cilium-assigned IP for the internet gateway. Envoy terminates TLS and routes to backend services.
...
DNS is **not** managed by the cluster. The Unifi router updates the `*.timosur.com` wildcard A record in Cloudflare to the router's public IP. All subdomains (`mealie.timosur.com`, `ai.timosur.com`, etc.) resolve via this wildcard. Home services use `*.home.timosur.com` which can be resolved via local DNS (e.g. Pi-hole).
...
│   └── internet/                         # Internet gateway (*.timosur.com)
...
- **Internet** (`envoy-gateway-internet`): Listeners on ports 80 and 443 for `*.timosur.com`, `timosur.com`, and `givgroov.de`. TLS terminated with certificates from cert-manager. HTTP automatically redirects to HTTPS.
...
Three policies enforce network segmentation:

1. **`home/gateway-lan-only.yaml`** — The home gateway only accepts ingress from LAN (`192.168.0.0/16`) and cluster-internal CIDRs. Prevents internet traffic from reaching home services even if port forwarding is misconfigured.

2. **`internet/gateway-isolation.yaml`** — Internet gateway pods cannot reach LAN subnets (`192.168.0.0/16`, `172.16.0.0/12`). Allows only DNS, pod CIDR, and service CIDR egress. Prevents lateral movement if a gateway pod is compromised.

3. **`internet/workload-egress-deny-lan.yaml`** — Cluster-wide policy. Any namespace labeled `exposure: internet` gets restricted egress — LAN subnets are blocked. Apply with:
   ```bash
